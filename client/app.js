...
window.addEventListener("wheel", (event) => {
  ...
  if (event.deltaY > 0) {
    ...
    start -= 2;
    if (thisScroll < 2 || thisScroll === NaN) {
      start = 0;
      return;
    }
    topContainer.style.top = `${start}%`;
...
function createTrackItem(track, playlist) {
  ...
  if (playlist !== undefined) {
    ...
    if (exists.length !== 0) {
      addToLibrary.children[0].classList.remove("fa-plus");
      addToLibrary.children[0].classList.add("fa-minus");
    }
  }
...
async function setPlayer(item, index) {
  itemIndex = index;
  let img, info, title, artist;
  if (!libOpen) {
    img = item.children[2].src;
    info = item.children[3];
  } else {
    img = item.children[1].src;
    info = item.children[2];
  }
  title = info.children[2].innerText;
  artist = info.children[0].innerText;
  cover.src = img;
  ...
  addToLibraryButtons.forEach((button) => {
    ...
    if (onlyDesktop) {
      ...
      button.addEventListener("mouseleave", function () {
        if (this.children[1].classList.contains("visible")) {
...
function editButtonsHandler() {
  ...
  editButtons.forEach((button) => {
    button.addEventListener("click", function () {
      editId = this.parentElement.id;
      let cover = this.parentElement.children[2].src;
      editImageCheckbox.checked = true;
      editImageTextInput.value = cover;
      let artist = this.parentElement.children[3].children[0].innerText;
      editArtistInput.value = artist;
      let title = this.parentElement.children[3].children[2].innerText;
