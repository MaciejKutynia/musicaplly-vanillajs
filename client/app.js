...
const audioLabel = document.querySelector("#audio-label p");
const imageLabel = document.querySelector("#image-label p");
...
audioInput.addEventListener("change", function () {
  ...
  audioLabel.innerText = this.files[0].name;
});

imageInput.addEventListener("change", function () {
  if (this.files.length === 0) {
    imageLabel.innerText = "Załącz plik lub upuść tutaj";
    return;
  }
...
submitButton.addEventListener("click", async function (event) {
  ...
  if (imageInput.files.length === 0) {
    ...
    cover = await readFile(imageInput.files[0]);
  }
...
async function fetchNewTrack(file) {
  ...
  await getLatest(skip);
}

//Get latest tracks
async function getLatest(index) {
  // FIXME: Adres do zmiany
  const data = await fetch("http://localhost:5000", {
    method: "POST",
    headers,
    body: JSON.stringify({ skip: index }),
  });
  const response = await data.json();
  if (data.status === 500) {
  ...
  if (localStorage.getItem("playlist") !== null) {
    libraryPlaylist = JSON.parse(localStorage.getItem("playlist"));
    removeAllChildNodes(libraryContainer);
    createLibrary(libraryPlaylist);
  } else {
    libraryPlaylist = undefined;
  }
  response.tracks.map((track) => {
    createTrackItem(track, libraryPlaylist);
  });
  trackItems = container.querySelectorAll(".track-item");
  libraryItems = libraryContainer.querySelectorAll(".track-item");
  ...
  const addToLibraryButtons = document.querySelectorAll(".add-library");
  ...
  if (container.children.length === response.length) {
...
async function playTrack(id) {
  ...
  audio.pause();
  //FIXME: adres do zmiany
  const data = await fetch("http://localhost:5000/track", {
    method: "POST",
    headers,
